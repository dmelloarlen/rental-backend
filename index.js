...
app.post('/rent', async (req, res) => {
  try {
    const { name, email, phone, date, days, course, user,vname,image } = req.body;
    const order = new Order({ name, email, phone, date, days, course, user,vname,image });
    await order.save();
...
app.post('/addproduct', upload.single('image'), async (req, res) => {
  try {
    const { name, vno, new_price, category, description,zip,user } = req.body;
    const image = req.file ? req.file.filename : ''; // Ensure image is correctly assigned

    if (!name || !vno || !new_price || !category) {
